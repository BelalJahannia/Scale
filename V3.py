...
class scaled_out_simulator:
    ...
    def run_simulation_single_layer(self, layer_id=0):
        ...
        def process_subarray(grid_row_id, grid_col_id):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(combinations)) as executor:
            ...
            # Wait for all subarrays to complete
        ...
        try:
            ...
            for size in gridsize:
                try:
                    ...
                    with open(output_file_path, 'a') as output_file:
                        output_file.write(
                            f'{top_File_Name[i]}, {top_File_Path[i]}, {size[0]}, {size[1]}, {cycles}, {util}, {ifmap_read}, {filter_reads}, {ofmap_writes}\n')
